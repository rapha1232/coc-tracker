...
import { SiegeCard } from "../UnitCard";
import { Label } from "../ui/label";
import { Switch } from "../ui/switch";

interface SiegesTabProps {
  sieges: Troop[];
  playerData: Main;
}

export default function SiegesTab({ sieges, playerData }: SiegesTabProps) {
  const [showCompleted, setShowCompleted] = useState(true);
  const completedSieges = sieges.filter((siege) => {
    const maxLevel = getMaxLevel(siege.name, playerData.townHallLevel) || 0;
    return siege.level === maxLevel;
  });

  const inProgressSieges = sieges.filter((siege) => {
    const maxLevel = getMaxLevel(siege.name, playerData.townHallLevel) || 0;
    return siege.level < maxLevel;
  });

  // Filtered list based on checkbox
  const visibleCompleted = showCompleted ? completedSieges : [];
